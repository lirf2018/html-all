		...
		<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
			<div class="coupon">
				<div class="coupon-list" v-for="(item, index) in couponList" :key="index">
					...
					<div @click="toPage(item.coupon_id,item.give_coupon_id)" class="coupon-use">
						<div>
							<span>点击查看</span>
						</div>
					</div>
	...
	export default {
		...
		methods: {
			onLoad() {
				...
				let params = {
					req_type: 'query_give_coupon_list',
					data: {
			...
			toPage(couponId, id) {
				this.$router.push('couponDetail?couponId=' + couponId + "&id=" + id);
			}
		}
	};
</script>
...
<style scoped>
	...
	.coupon-use>div>span:first-child {
		color: #ffffff;
