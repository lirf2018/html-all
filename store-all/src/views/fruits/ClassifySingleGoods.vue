		...
		<div class="classfy-contents">
			<div class="left-menu" :style="myStyle">
				<van-sidebar v-model="activeKey" @change="onChange">
					<van-sidebar-item :style="{'color':item.menuId == onclickClassifyId ?'#008000':''}"
						:index="item.menuId" :title="item.menuName" v-for="(item, index) in menuList"
						:disabled="item.isdisabled" :key="item.menuId" />
				</van-sidebar>
			</div>
							...
							<div class="goods-items" v-for="(item, index) in goodsList" :key="item.goodsId">
								...
								<div class="goods-contents">
									...
									<div class="price-div">
										...
										<div class="add-cart">
											<div v-if="item.userGoodsCount == 0" class="shop-cart"
												@click="onclickShopCart(item.goodsId)">
												<van-icon name="cart-circle-o" color="#008000;" />
											</div>
											<div v-if="item.userGoodsCount > 0">
												<van-stepper input-width="20px" button-size="12px"
													v-model="item.userGoodsCount" disable-input
				...
				setTimeout(() => {
					...
					for (let i = 0; i < 10; i++) {
						...
						var data = {
							...
							goodsDesc: '皮薄，口感特别好。果肉细腻，看的出是新鲜采摘下来的，个子很大个，包装严实，没有坏果，皮薄肉厚，水分充足，好吃还不贵，苹果红红的，看着就很新鲜，苹果闻起来也是香气十足，颜色鲜艳好看，脆爽可口，汁多皮薄，味道极好。果子都很新鲜香味扑鼻个个都是熟透的香甜可口的大苹果。',
							userGoodsCount: 0,
							goodsPrice: '30.23',
