	...
	<div class="body-bg">
		...
		<div class="addrs-list">
			...
			<div v-for="(item, index) in list" :key="index">
				...
				<div class="user-info"  @click="setParentUserChoseAddr(item.id)">
					<div>
		...
		<van-popup
		  ...
		  :style="{ height: '100%' }"
		>
		<UserAddrAdd :setShowUserAddrAddFlag = "setShowUserAddrAddFlag"></UserAddrAdd>
		</van-popup>
	</div>
</template>
...
<script>
	...
	import UserAddrAdd from '@/components/UserAddrListAddPannel.vue'
	import {
		Dialog,
		Toast
	} from 'vant';
	export default {
		components: {
			UserAddrAdd: UserAddrAdd
		},
		props:["setShowAddrListPannelFlag","setUserChoseAddr"],
		data() {
			return {
				title: '收货地址',
				chosenAddressId: '1',
				list: [],
				showAddrListAddPannelFlag:false
			};
		},
		mounted: function() {
		...
		methods: {
			...
			showUserAddrAddFlag() {
				if(this.showAddrListAddPannelFlag){
					this.showAddrListAddPannelFlag = false;
				}else{
					this.showAddrListAddPannelFlag = true;
				}
			},
			setShowUserAddrAddFlag(){
				this.findAddrList();
				this.showAddrListAddPannelFlag = false;
			},
			closeShowAddrListAddPannelFlag(){
				this.setShowAddrListPannelFlag();
			},
			setParentUserChoseAddr(userAddrId){
				let vm = this;
				vm.list.forEach(function(e){
					if(userAddrId == e.id){
						let obj = {
							...
							phone : e.user_phone,
							addrDetail : e.addr_name,
							postPrice : e.freight
						}
						vm.setUserChoseAddr(obj);
						vm.closeShowAddrListAddPannelFlag();
					}
				})
