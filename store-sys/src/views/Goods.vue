<template>
	<div class="page">
		...
		<div>
			...
			<div class="list">
				<Table size="large" border ref="selection" :columns="listTitle" :data="listContents" @on-selection-change="onSelectionChange">
					<template slot-scope="{ row }" slot="name">
						<strong>操作</strong>
					</template>
					<template slot-scope="{ row, index }" slot="action">
						<Button v-if="row.status == 2 || row.status == 0" @click="deleteGoods(row.goods_id)" style="margin-right: 15px">删除</Button>
						<Button v-if="row.status == 1" style="margin-right: 15px" @click="updateGoodsStatus(row.goods_id, 2)">下架</Button>
						<Button v-if="row.status == 2 || row.status == 0" style="margin-right: 15px" @click="updateGoodsStatus(row.goods_id, 1)">上架</Button>
		...
		<div>
			<Modal v-model="addGoodsFlag" :title="title" :fullscreen="true">
				<div class="goods-info-div">
					...
					<Form ref="addFormData" :model="addFormData" :label-width="300" :rules="ruleValidate">
						...
						<div class="item">
							<FormItem label="商品名称" prop="goodsPy"><Input placeholder="商品名称" v-model="addFormData.goodsName" /></FormItem>
						</div>
						<div class="item">
							<FormItem label="拼音码" prop="goodsPy"><Input placeholder="拼音码" v-model="addFormData.goodsPy" /></FormItem>
						</div>
						<div class="item">
						...
						<div class="item">
							<FormItem label="促销开始时间" prop="discountsStartTimeShow">
								<DatePicker type="date" placeholder="促销开始时间" :editable="false" v-model="addFormData.discountsStartTimeShow"></DatePicker>
							</FormItem>
						</div>
						<div class="item">
							<FormItem label="会员价格" prop="memberPrice"><Input placeholder="会员价格" number v-model="addFormData.memberPrice" /></FormItem>
						</div>
						<div class="item">
							<FormItem label="促销结束时间" prop="discountsEndTimeShow">
								<DatePicker type="date" placeholder="促销结束时间" :editable="false" v-model="addFormData.discountsEndTimeShow"></DatePicker>
							</FormItem>
						</div>
						<div class="item">
							<FormItem label="促销价格" prop="discountsPrice"><Input placeholder="促销价格" number v-model="addFormData.discountsPrice" /></FormItem>
						</div>
						...
						<div class="item">
							<FormItem label="商品规格"><Input placeholder="商品规格" disabled v-model="addFormData.goodsUnitName" /></FormItem>
						</div>
						<div class="item">
							<FormItem label="是否促销">
				...
				<div>
					<div class="list">
						<Table size="large" border :columns="listStoreTitle" :data="listStoreContents" @on-row-click="addStoreDetail">
							<template slot-scope="{ row }" slot="name">
								<strong>操作</strong>
							</template>
							<template slot-scope="{ row, index }" slot="action">
			...
			<Modal v-model="modal3" width="750px" title="库存详情">
				<div class="list"><Table size="large" border :columns="listStoreInfoTitle" :data="listStoreInfoContents"></Table></div>
			</Modal>
...
export default {
	...
	data() {
		return {
			...
			ruleValidate: {
				goodsPy: [{ required: true, type: 'string', message: '拼音不能为空', trigger: 'blur' }],
				salePrice: [{ required: true, type: 'number', message: '销售价格式有误, 数字', trigger: 'blur' }],
				memberPrice: [{ type: 'number', message: '会员价格式有误, 数字', trigger: 'blur' }],
				discountsPrice: [{ type: 'number', message: '促销价格式有误, 数字', trigger: 'blur' }],
				storeWarning: [{ required: true, type: 'integer', message: '库存预警值只能是数字', trigger: 'blur' }],
				discountsStartTimeShow: [{ type: 'date', message: '日期格式有误, 例如2020-01-01', trigger: 'blur' }],
				discountsEndTimeShow: [{ type: 'date', message: '日期格式有误, 例如2020-01-01', trigger: 'blur' }]
			},
			// 结果
			listTitle: [],
			listContents: [],
	...
	created() {
		this.listTitle = [
			...
			{ title: '标识', key: 'goods_id', align: 'center', width: 80 },
			{ title: '商品条形码', key: 'goods_code', align: 'center',width:180 },
			{ title: '商品名称', key: 'goods_name', align: 'center' },
			{ title: '商品拼音', key: 'goods_py', align: 'center',width: 110 },
			{ title: '商品规格', key: 'goods_unit_name', align: 'center',width: 110 },
			{ title: '规格数量', key: 'unit_count', align: 'center',width: 110 },
			{ title: '库存', key: 'store', align: 'center',width: 80 },
			{ title: '库存预警值', key: 'store_warning', align: 'center' ,width: 130},
			{ title: '销售价格', key: 'sale_price', align: 'center',width: 110 },
			{ title: '会员价格', key: 'member_price', align: 'center',width: 110 },
			{
				title: '促销价格',
				key: 'discounts_price',
				align: 'center',
			...
			{ title: '操作', slot: 'action', align: 'center', width: 280 }
		];

		this.listStoreTitle = [
			{ title: '标识', key: 'income_id', align: 'center' },
			{ title: '商品名称', key: 'goods_name', align: 'center' },
		...
		this.listStoreInfoTitle = [
			...
			{ title: '入库时间', key: 'in_time_name', align: 'center', width: 220 }
		];
	},
	mounted: function() {
