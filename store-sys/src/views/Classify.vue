		...
		<div>
			<div class="search-form">
				<Form :model="searchForm" :label-width="100" :inline="true">
					<FormItem label="分类名称"><Input placeholder="分类名称" v-model="searchForm.classify_name"
							@keyup.native="clickSearch()" /></FormItem>
					<FormItem label="分类编码"><Input placeholder="分类编码" v-model="searchForm.classify_code"
							@keyup.native="clickSearch()" /></FormItem>
			...
			<div class="in-btn"><Button style="margin-right: 20px" @click="showAddModel(0, '', '', '')">添加分类</Button>
			</div>
			<div class="list">
				<Table size="large" border ref="selection" :columns="listTitle" :data="listContents">
					...
					<template slot-scope="{ row, index }" slot="action">
						<Button style="margin-right: 15px"
							@click="showAddModel(row.classify_id, row.classify_name, row.classify_code, row.classify_sort)">修改</Button>
		...
		<div>
			<Modal v-model="addClassifyFlag" :title="title" @on-ok="saveData">
				<Form ref="addForm" :model="addFormData" :label-width="100" :rules="ruleValidate">
					...
					<div class="item">
						<FormItem label="分类名称" prop="classifyName"><Input placeholder="分类名称"
								v-model="addFormData.classifyName" /></FormItem>
					</div>
					<div class="item">
						<FormItem label="分类编码" prop="classifyCode"><Input placeholder="分类编码"
								v-model="addFormData.classifyCode" /></FormItem>
					</div>
					<div class="item">
						<FormItem label="排序" prop="classifySort"><Input string placeholder="排序"
								v-model="addFormData.classifySort" /></FormItem>
					</div>
				</Form>
			</Modal>
