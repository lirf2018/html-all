		...
		<Modal :fullscreen="fullscreen" v-model="addGoodsFlag" title="添加商品信息">
			<div class="append-goods">
				...
				<div class="div3">
					...
					<div class="page-next">
						<Page
							:current="searchGoodsForm.curre_page"
							show-total
							:page-size="searchGoodsForm.page_size"
							:total="total"
							show-sizer
							prev-text="上一页"
							next-text="下一页"
			...
			<div class="">
				...
				<div class="more-order-content">
					<div
						...
						v-for="(item, index) in orderGoodsLists"
						:key="index"
						:class="item.style"
						@mouseover="mouseInGoodsUl(item.orderNum, 2)"
			...
			axios.post('', params).then(function(res) {
				if (res.resp_code == 1) {
					...
					if(vm.goodsSearchList.length == 0){
						vm.$Message.warning({ content: '商品不存在', duration: 5 });
