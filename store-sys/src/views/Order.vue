	...
	<div class="page">
		...
		<div class="search-form">
			<Form :model="searchForm" :label-width="120" :inline="true">
				<FormItem label="订单号"><Input placeholder="订单号" v-model="searchForm.order_no"
						@keyup.native="clickSearch()" /></FormItem>
				<FormItem label="手机号"><Input placeholder="手机号" v-model="searchForm.user_phone"
						@keyup.native="clickSearch()" /></FormItem>
				<FormItem label="桌号"><Input placeholder="桌号" v-model="searchForm.table_name"
						@keyup.native="clickSearch()" /></FormItem>
				...
				<FormItem label="状态">
					<Select placeholder="选择状态" v-model="searchForm.order_status" @on-change="clickSearch">
						<Option :value="item.param_key" v-for="(item, index) in orderStatus" :key="index">
							{{ item.param_value }}
						</Option>
					</Select>
				</FormItem>
