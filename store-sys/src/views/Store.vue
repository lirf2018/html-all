		...
		<div>
			<Modal v-model="addGoodsFlag" :title="title" :fullscreen="true" @on-ok="saveData(0)">
				...
				<Form ref="formValidate" :model="addFormData" :label-width="250" :rules="ruleValidate">
					...
					<div class="item">
						<FormItem label="截止类型">
							<Select v-model="addFormData.endTimeType">
								<Option value="0">截止日期</Option>
								<Option value="1">有效年</Option>
								<Option value="2">有效月</Option>
								<Option value="3">有效天</Option>
							</Select>
						</FormItem>
					</div>
					<div class="item">
...
<script>
	import COm from '@/commons/common.js'
	...
	export default {
		...
		methods: {
			...
			checkSubmitData() {
				...
				debugger
				if (vm.addFormData.endTimeType == null) {
					const msg = '截止类型不能为空';
					vm.$Message.warning({
						content: msg,
						duration: 5
				...
				if (vm.addFormData.endTimeType == 0 && vm.addFormData.effectToTime == null) {
					const msg = '有效截止日期不能为空';
					vm.$Message.warning({
						content: msg,
						duration: 5
					});
				...
				if (vm.addFormData.endTimeType == 1 && vm.addFormData.year == null) {
					const msg = '有效年不能为空';
					vm.$Message.warning({
						content: msg,
						duration: 5
				...
				if (vm.addFormData.endTimeType == 2 && vm.addFormData.month == null) {
					const msg = '有效月不能为空';
					vm.$Message.warning({
						content: msg,
						duration: 5
				...
				if (vm.addFormData.endTimeType == 3 && vm.addFormData.effectDay == null) {
					const msg = '有效天数不能为空';
					vm.$Message.warning({
						content: msg,
						duration: 5
				...
				if (vm.addFormData.year != null && vm.addFormData.year < 0) {
					const msg = '有效年数为不小于0的整数';
					vm.$Message.warning({
						content: msg,
						duration: 5
					});
				...
				if (vm.addFormData.month != null && vm.addFormData.month < 0) {
					const msg = '有效月数为不小于0的整数';
					vm.$Message.warning({
						content: msg,
						duration: 5
					});
				...
				if (vm.addFormData.effectDay != null && vm.addFormData.effectDay < 0) {
					const msg = '有效天数为不小于0的整数';
					vm.$Message.warning({
						content: msg,
						duration: 5
					});
